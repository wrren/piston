...
namespace piston
{
    ...
    memory_region::iterator memory_region::iterator::operator+(size_t advance) const
    {
        return iterator(m_address + advance, m_iterator + advance);
    }

    memory_region::iterator& memory_region::iterator::operator+=(size_t advance)
    {
        m_iterator += advance;
        m_address += advance;
        return *this;
    ...
    const byte* memory_region::iterator::get_data() const
    {
        return &(*m_iterator);
    }
    ...
    memory_region::memory_region(iterator begin, iterator end) :
    m_base_address(begin.get_address()),
    ...
    memory_region::diff_type memory_region::diff(const memory_region& other) const
    {
        ...
        if(overlaps_with(other))
        {
            ...
            for(auto m_it = m_begin, o_it = o_begin; m_it != m_end && o_it != o_end; ++m_it, ++o_it)
            {
                ...
                {
                    diff.push_back(memory_region(region_start, o_it));
                    region_start    = o_it;
                    region_size     = 0;
                }
                else if(*m_it != *o_it)
            ...
            if(region_size > 0)
            {
                diff.push_back(memory_region(region_start, ++region_end));
            }
        }
